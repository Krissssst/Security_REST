...
//Прописываем функцию заполнение таблицы
    ...
    $('allUsersTableBody').empty()
        ...
        .then(data => {
                ...
                data.forEach(element => {
                    ...
                    <td>${element.roles.map(role => role.role.substring(5))}</td>
                    
                    <td>
                    <button type="button" class="btn btn-dander delete" id="buttonDelete"
...
//Прописываем функцию заполнения текущего пользователя
export function fullCurrentUserTable() {

    const currentUserTableBody = document.getElementById('currentUserTableBody')
    const currentLoginUser = document.getElementById('currentLoginUser').innerText
    ...
    fetch(url)
        ...
        .then(data => {
            let columnContent = ''
            data.forEach(element => {
                    if (element.username == currentLoginUser) {
                        columnContent += `<tr>
                    <td>${element.id}</td>
                    <td>${element.name}</td>
                    <td>${element.surname}</td>
...
//Функция выбора ролей для нового пользователя
